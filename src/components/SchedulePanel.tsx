    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-gray-50">
        ...
        <div className="text-center">
          <h2 className="font-semibold text-gray-900">Schedule Builder</h2>
      ...
      <div className="px-4 py-2 bg-red-50 border-b border-red-100">
        <p className="text-xs text-red-800 flex items-center gap-1">
          ...
          Click & drag to block times you&apos;re unavailable
        </p>
      </div>
      ...
      <div className="flex-1 overflow-auto p-4">
        <div className="min-w-[320px]">
          {/* Days Header */}
          <div className="grid grid-cols-[52px_repeat(5,1fr)] gap-0 mb-1 sticky top-0 bg-white z-20">
            <div></div>
            {DAY_LABELS.map((day, i) => (
              <div
                key={DAYS[i]}
                className="text-[10px] font-semibold text-gray-700 text-center py-1 border-b border-gray-200"
              >
                {day}
              </div>
            ))}
          ...
          <div className="relative select-none">
            {TIME_SLOTS.map(({ hour, minute }, slotIndex) => {
              ...
              return (
                <div key={`${hour}-${minute}`} className="grid grid-cols-[52px_repeat(5,1fr)] gap-0">
                  {/* Time label - show for every slot */}
                  <div className={`text-[8px] text-gray-400 text-right pr-1 flex items-center justify-end`}
                    style={{ height: `${SLOT_HEIGHT}px` }}
                  >
                    <span className={isHourStart ? 'text-gray-500 font-medium' : ''}>
                      {hour > 12 ? hour - 12 : hour === 0 ? 12 : hour}:{minute.toString().padStart(2, '0')} {hour >= 12 ? 'pm' : 'am'}
                    </span>
                  </div>
      ...
      <div className="p-3 border-t border-gray-200 bg-gray-50 space-y-2">
        <div className="flex justify-between text-xs text-gray-600">
          ...
          <span className="font-semibold text-gray-900">{totalCredits} credits</span>
        </div>
